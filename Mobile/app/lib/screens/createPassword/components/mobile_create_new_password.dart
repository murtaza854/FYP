...
import '../../otpPhone/otp_phone_screen.dart';

class MobileCreateNewPassword extends StatefulWidget {
  const MobileCreateNewPassword({Key? key}) : super(key: key);

  @override
  State<MobileCreateNewPassword> createState() =>
      _MobileCreateNewPasswordState();
}

class _MobileCreateNewPasswordState extends State<MobileCreateNewPassword> {
  ...
  String buttonText = 'Create Password';
                    ...
                    if (_formKey.currentState!.validate()) {
                      ...
                      try {
                        if (buttonText == 'Processing...') {
                          ...
                          AuthCredential credential = EmailAuthProvider.credential(email: user?.email ?? '', password: globals.user?.tempPassword ?? '');
                          await user?.reauthenticateWithCredential(credential);
                          await user?.updatePassword(_passwordController.text);
                          final response = await http.post(
                              Uri.parse(
                                  "${dotenv.env['API_URL1']}/user/password-created"),
